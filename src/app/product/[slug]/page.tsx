...
import { getProductSlugs, getProductSource, getProductMeta } from "@/lib/product";
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  if (!getProductSlugs().includes(slug)) notFound();
  const { frontmatter, content } = await renderMdx(getProductSource(slug), mdxComponents);
